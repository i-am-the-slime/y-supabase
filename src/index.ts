...
export default class SupabaseProvider extends EventEmitter {
    ...
    private readonly resyncInterval: NodeJS.Timer | undefined;
    private readonly debounceUpdate: () => void;
    ...
    constructor(private doc: Y.Doc, private supabase: SupabaseClient, private config: SupabaseProviderConfig) {
        ...
        if (!this.resyncInterval) {
            this.logger(`setting resync interval to every ${(this.config.resyncInterval || 100) / 1000} seconds`);
            ...
            }, this.config.resyncInterval || 100);
        }
        ...
        this.on('awareness', async (update) => {
            if (this.channel)
